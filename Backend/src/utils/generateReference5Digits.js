...
export function generateReference5Digits(prefix, fixedLetter, number) {
  try {
    ...
    internalServerError(
      error.message || 'Error al generar la referencia de 5 dígitos'
    );
  }
}
...
export function generateReference5DigitsFromRef(prefix, maxRef) {
  try {
    ...
    internalServerError(
      error.message || 'Error al generar la referencia desde la referencia máxima existente'
    );
  }
}
